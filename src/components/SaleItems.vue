<template>
  <div class="max-w-screen-xl mx-auto">
    <div class="flex justify-between items-center my-6 md:m-6 px-6 lg:px-12">
      <h2
      ...
      <div v-for="product in saleItems" :key="product.id" class="flex flex-col">
        ...
        <div class="relative w-full pb-[125%]">
          <img
            ...
            :alt="product.title"
          />
          <div class="absolute top-2 left-2 bg-mahogany text-white text-sm px-2 py-1 rounded">
            SALE
          </div>
        </div>
        <!-- Product info -->
        <div class="mt-4">
          <div class="flex justify-between">
            <p class="text-base font-medium text-gray-600 truncate mr-2">{{ product.title }}</p>
            <p class="text-base font-semibold text-gray-600 whitespace-nowrap">
              {{ product.price }} KR
            </p>
          </div>
        </div>
