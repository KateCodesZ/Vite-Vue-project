<template>
  <div class="max-w-screen-xl mx-auto">
    <div class="flex justify-between items-center my-8 md:m-6 px-6 lg:px-12">
      <h2 class="text-3xl md:text-4xl lg:text-5xl text-marianblue font-semibold">
        <span class="font-montserrat">N</span>
        <span class="font-montserrat-alt">yheter</span>
      </h2>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-8 px-4 mb-12 my-10">
      <div v-for="product in newItems" :key="product.id" class="flex flex-col">
        ...
        <div class="relative w-full pb-[125%]">
          <img
            ...
            :alt="product.title"
          />
          <div class="absolute top-2 left-2 bg-marianblue text-white text-sm px-2 py-1">NEW</div>
        </div>
...
<script setup>
  ...
  const newItems = computed(() => store.products.filter((product) => product.isNew).slice(0, 4))
</script>
